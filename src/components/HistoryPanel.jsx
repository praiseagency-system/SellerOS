...
import { Trash2, Download, Upload, Clock, FolderOpen } from 'lucide-react'
...
export default function HistoryPanel({ sessions, onClose, onSessionsChange, onLoad }) {
  ...
  return (
    <Modal title="Riwayat Periode" subtitle={`${sessions.length} periode tersimpan`}
      onClose={onClose} maxWidth="max-w-md">
      <div className="p-4 space-y-3">
        {/* Import JSON */}
        <div className="flex justify-end">
          <button onClick={() => importRef.current?.click()} title="Import JSON"
            className="flex items-center gap-1 text-xs text-ink-muted hover:text-ink border border-line/10 rounded-lg px-2.5 py-1.5">
        ...
        <p className="text-xs text-ink-faint leading-relaxed border-t border-line/5 pt-3">
