...
export async function generateDecisionIntelligence({ workspaceId, storeId, date, generatedAt, persist = false, db, sb, skills = [1, 2, 3, 4, 9] }) {
  ...
  const byNum = { 1: skill1, 2: skill2, 3: skill3, 4: skill4, 9: skill9 }
  ...
  const result = {
    ...
    daily, skill1, skill2, skill3, skill4, skill9,
...
async function persistOutputs({ sb, result, gen }) {
  ...
  const rows = [result.skill1, result.skill2, result.skill3, result.skill4, result.skill9].map(o => skillRow(o, result, gen))
