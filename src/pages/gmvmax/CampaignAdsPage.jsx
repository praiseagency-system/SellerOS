...
export default function CampaignAdsPage({ onOpenUpload }) {
  ...
  const [detail, setDetail] = useState(null) // campaign → sub-halaman matriks status
  ...
  return (
    <div className="p-6 space-y-4 max-w-7xl mx-auto">
      ...
      {(() => {
        const open = (action, m) => {
          if (action === 'DETAIL') { setDetail(m.s || { campaign_id: m.id, campaign_name: m.name }); return }
          setQueuedMsg(null); setDialog({ action, campaign: m.s })
        }
        const act = merged.filter(m => isOn(m.s?.operation_status))
      ...
      {s?.campaign_id && onAction && (
        <div className="flex items-center gap-1.5 mb-3 flex-wrap">
          ...
          <button onClick={() => onAction('DETAIL')}
            className="px-2.5 py-1.5 rounded-lg text-[11px] font-semibold border border-blue-500/30 text-blue-300 hover:bg-blue-500/10 transition-colors">
            Detail produk
          </button>
