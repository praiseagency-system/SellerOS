// Perbandingan channel GMV Max: Video · Product card · Live. Kartu ringkas +
// share bar + tren harian stacked + tabel rincian. Channel = rollupChannels
// (Live dipisah dari Card via sinyal campaign LIVE).
import {
  ResponsiveContainer, BarChart, Bar, XAxis, YAxis, Tooltip, CartesianGrid,
} from 'recharts'
import { PlayCircle, LayoutGrid, Radio, Info } from 'lucide-react'
import { useGmvMax } from '../../contexts/GmvMaxContext'
import { fmtRp, fmtRpC, fmtRoasX, DeltaBadge, EmptyState } from '../../components/gmvmax/ui'
...
export default function ChannelPage({ onOpenUpload }) {
  const { channels: ch, channelTrend, prev, periodName, hasData } = useGmvMax()
  ...
  const trend = channelTrend.map(d => ({ ...d, name: d.date.slice(8) })) // tanggal (DD)
    ...
    <div className="p-6 space-y-4 max-w-6xl mx-auto">
      {periodName && (
        <p className="text-sm text-ink-muted -mb-1">
          ...
          <span className="text-ink-faint"> · total {fmtRp(total)}</span>
          {prev && <span className="text-ink-faint"> · vs {prev.name}</span>}
        </p>
      )}
